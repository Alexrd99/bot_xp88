...
from fungsi import FunctionHandler
import sys
import subprocess
...
class UiXP(QMainWindow):
    ...
    def update_menu_kiri(self):
        for device, checkbox in self.menu_kiri_dict.items():
            checkbox.stateChanged.connect(lambda state, d=device: self.function_handler.cb_device(state, d))
    ...
    def populate_menu_kiri(self):
        ...
        for index, device in enumerate(devices):
            checkbox = QCheckBox(f"{index + 1} - {device}")  # Tambahkan teks ke checkbox
